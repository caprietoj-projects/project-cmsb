...
function sanbartolome_theme_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'link_descarga', array(
        'type' => 'theme_mod',
        'sanitize_callback' => $pgwp_sanitize
    ));

    $wp_customize->add_control( 'link_descarga', array(
        'label' => __( 'Link Descargar Recibo', 'sanbartolome_theme' ),
        'type' => 'url',
        'section' => 'pagina_informacion_padres_familia'
    ));

    $wp_customize->add_setting( 'id_descarga', array(
        'type' => 'theme_mod',
        'sanitize_callback' => $pgwp_sanitize
    ));

    $wp_customize->add_control( new WP_Customize_Media_Control( $wp_customize, 'id_descarga', array(
...
if ( ! function_exists( 'sanbartolome_theme_enqueue_scripts' ) ) :
    ...
    wp_enqueue_style( 'sanbartolome_theme-style-2', get_bloginfo('stylesheet_url'), false, null, 'all');
